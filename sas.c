...
void acheterProduit(){
	...
	if(rechercherParCode(produit.code)!=-1){
		...
		if(stockProduits[rechercherParCode(produit.code)].quantite>=produit.quantite){
			...
			ventes[indiceVente].date.jour=21;
			ventes[indiceVente].date.mois=9;
			ventes[indiceVente].date.annee=2022;
			indiceVente++;
	        enregistrerVente();
...
void enregistrerVente(){
    ...
    if (fichier != NULL)
    {
    	...
    	for(i=0;i<indiceVente;i++){
	       ...
	       fprintf(fichier, "%d %s %d %.2f\n", ventes[i].produit.code, ventes[i].produit.nom, ventes[i].produit.quantite,ventes[i].produit.prix);
